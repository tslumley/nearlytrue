...
x<-rnorm(10000)
mu<-expit(x*beta-beta^2/2-3)
pi0<- sum(mu)/(10000-sum(mu))
...
X<-cbind(1,x)
mu1<-expit(x*beta-beta^2/2-3-log(pi0))

V<- t(X)%*%(mu*(1-mu)*(X))/10000
...
yy<-y[c(cases,ctrls)]
xx<-x[c(cases,ctrls)]
c(coef(glm(y~x,family=binomial)),coef(glm(yy~xx,family=binomial)),
